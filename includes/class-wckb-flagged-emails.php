<?php
/**
 * WCKB Flagged Emails Management
 *
...
class WCKB_Flagged_Emails {
    ...
    public function __construct() {
        global $wpdb;
        $this->table_name = $wpdb->prefix . 'wckb_flagged_emails';
    }
     ...
     */
    public function flag_email( $email, $kickbox_result, $order_id = null, $user_id = null, $origin = 'checkout' ) {
        global $wpdb;
     ...
     * @param string $decision Admin decision (pending, allow, block)
     * @return object|null Flagged email object or null if not found
     */
    public function get_flagged_email_by_email( $email, $decision = 'pending' ) {
        global $wpdb;

        $result = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM {$this->table_name} WHERE email = %s AND admin_decision = %s ORDER BY flagged_date DESC LIMIT 1",
    ...
    public function get_admin_decision( $email ) {
        $flagged_email = $this->get_flagged_email_by_email( $email, 'allow' );
        if ( $flagged_email ) {
            return 'allow';
        }

        $flagged_email = $this->get_flagged_email_by_email( $email, 'block' );
        if ( $flagged_email ) {
            return 'block';
        }
    ...
    /**
     * Check if email has pending review
     *
     * @param string $email Email address
     * @return bool True if email has pending review
     */
    public function has_pending_review( $email ) {
        $flagged_email = $this->get_flagged_email_by_email( $email, 'pending' );
        return ! empty( $flagged_email );
    }
